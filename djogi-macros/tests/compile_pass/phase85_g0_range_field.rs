    ...
    let _all = RangeAllSubtypes::objects().filter(|f| {
        f.i4().explicit_pg_predicate().contains(3_i32)
            & f.i4()
                ...
                .contains_range(probe.clone())
            & f.i4()
                .explicit_pg_predicate()
                .contained_by(envelope.clone())
            & f.i4().explicit_pg_predicate().overlaps(probe.clone())
            & f.i4()
                .explicit_pg_predicate()
                .strictly_left_of(envelope.clone())
            & f.i4()
                .explicit_pg_predicate()
                .strictly_right_of(probe.clone())
            & f.i4()
                .explicit_pg_predicate()
                .not_extends_right_of(envelope.clone())
            & f.i4()
                .explicit_pg_predicate()
                .not_extends_left_of(probe.clone())
            & f.i4().explicit_pg_predicate().adjacent_to(probe.clone())
    });
