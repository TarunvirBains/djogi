...
#[model(table = "temporal_rows", pk = HeerId, no_default)]
...
#[djogi::djogi_test(sync_models = [TemporalRow])]
async fn temporal_year_check_in_range_round_trip(mut ctx: djogi::DjogiContext) {
    ...
    let fetched = TemporalRow::get(&mut ctx, row.id)
        .await
...
async fn temporal_year_check_rejects_oob_date(mut ctx: djogi::DjogiContext) {
    ...
    // "temporal_rows_event_on_check"` violation. The
    ...
    assert!(
        msg.contains("temporal_rows_event_on_check"),
...
async fn temporal_year_check_rejects_oob_timestamptz(mut ctx: djogi::DjogiContext) {
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("temporal_rows_recorded_at_check"),
