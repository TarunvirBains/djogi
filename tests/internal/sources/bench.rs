// Release-mode smoke benchmarks for the
// `convex_hull` aggregate + `.qualify(...)` derived-table
// lowering surface.
//
...
//
// ```bash
// cargo test --test bench -p djogi --all-features \
...
//
// Same rationale as pool_bench and tree_query_bench:
...
// not need `spatial`, but co-locating both benches in one file
// keeps the bench fixture single-file and matches how the two
