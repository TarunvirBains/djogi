...
//
// an internal review round polish: the probe's `None` branch must hard-error
...
async fn metadata_only_segment_accounted_in_run_report(mut ctx: djogi::DjogiContext) {
  ...
  // an internal review round polish: the metadata segment's `up` SQL is a
  ...
  // which is the bug is guarding against.
...
async fn duplicate_version_surfaces_typed_error(mut ctx: djogi::DjogiContext) {
  ...
  match err {
    RunnerError::VersionAlreadyApplied {
      ...
      // regression of the fix where the orchestrator falls
