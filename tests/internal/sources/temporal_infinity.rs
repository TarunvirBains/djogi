...
//   (djogi#187) + "`Range<T>` typed substrate (djogi#215, G0)".
...
#[model(table = "temporal_inf_date_scalar", pk = HeerId, no_default)]
...
#[model(table = "temporal_inf_tstz_scalar", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
...
#[model(table = "temporal_inf_date_range", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
...
/// endpoint CHECKs.
#[model(table = "temporal_inf_tstz_range", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
...
async fn scalar_date_check_rejects_positive_infinity(mut ctx: djogi::DjogiContext) {
    ...
    let err = ctx
        .raw_execute(
            "INSERT INTO temporal_inf_date_scalar (event_on, label) \
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("temporal_inf_date_scalar_event_on_check"),
...
async fn scalar_tstz_check_rejects_positive_infinity(mut ctx: djogi::DjogiContext) {
    let err = ctx
        .raw_execute(
            "INSERT INTO temporal_inf_tstz_scalar (recorded_at, label) \
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("temporal_inf_tstz_scalar_recorded_at_check"),
...
async fn daterange_check_rejects_negative_infinity_in_lower_endpoint(
    ...
    let err = ctx
        .raw_execute(
            "INSERT INTO temporal_inf_date_range (validity, label) \
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("temporal_inf_date_range_validity_check"),
...
async fn tstzrange_check_rejects_negative_infinity_in_lower_endpoint(
    ...
    let err = ctx
        .raw_execute(
            "INSERT INTO temporal_inf_tstz_range (booking, label) \
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("temporal_inf_tstz_range_booking_check"),
        "-infinity lower-endpoint error must reference the structural CHECK constraint name: \
         {msg}"
    );
}
...
#[model(table = "temporal_inf_date_array", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
...
async fn date_array_check_rejects_negative_infinity_element(mut ctx: djogi::DjogiContext) {
    ...
    let err = ctx
        .raw_execute(
            "INSERT INTO temporal_inf_date_array (dates, label) \
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("temporal_inf_date_array_dates_check"),
...
#[model(table = "temporal_inf_tstz_array", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
...
async fn tstz_array_check_rejects_negative_infinity_element(mut ctx: djogi::DjogiContext) {
    let err = ctx
        .raw_execute(
            "INSERT INTO temporal_inf_tstz_array (timestamps, label) \
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("temporal_inf_tstz_array_timestamps_check"),
