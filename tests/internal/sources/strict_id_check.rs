...
#[model(table = "strict_default_off", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictDefaultOff {
    pub label: String,
}
...
#[model(
    table = "strict_strict_heer",
    pk = HeerId,
    strict_ids,
    no_default,
)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictStrictHeer {
    pub label: String,
}
...
#[model(
    table = "strict_strict_ranj",
    pk = RanjId,
    strict_ids,
    no_default,
)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictStrictRanj {
    pub label: String,
}

// ── (4) Field-level opt-in on a bare HeerId user column ──────────────────────

#[model(table = "strict_field_optin", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictFieldOptin {
...
#[model(table = "strict_fk_target", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictFkTarget {
    pub label: String,
}

#[model(
    table = "strict_fk_source",
    pk = HeerId,
    strict_ids,
    no_default,
)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictFkSource {
    ...
    /// the suffix).
    pub owner_id: ForeignKey<StrictFkTarget>,
    pub label: String,
}
...
// `StrictCustomBigintId` — custom BIGINT-shaped PK simulating an
...
djogi::primary_key! {
    pub struct StrictCustomBigintId(i64);
    sql_type = "BIGINT";
    default_sql = "txid_current()";
}

#[model(table = "strict_custom_bigint_target", pk = StrictCustomBigintId, no_default)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictCustomBigintTarget {
    pub label: String,
}

#[model(
    table = "strict_custom_bigint_fk_source",
    pk = HeerId,
    strict_ids,
    no_default,
)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictCustomBigintFkSource {
    ...
    /// target's PK).
    pub owner_id: ForeignKey<StrictCustomBigintTarget>,
    pub label: String,
}
...
djogi::primary_key! {
    pub struct StrictCustomUuidId(::uuid::Uuid);
    sql_type = "UUID";
    default_sql = "gen_random_uuid()";
}

#[model(table = "strict_custom_uuid_target", pk = StrictCustomUuidId, no_default)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictCustomUuidTarget {
    pub label: String,
}

#[model(
    table = "strict_custom_uuid_fk_source",
    pk = HeerId,
    strict_ids,
    no_default,
)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictCustomUuidFkSource {
    ...
    pub owner_id: ForeignKey<StrictCustomUuidTarget>,
    pub label: String,
}
...
#[model(table = "strict_field_fk_custom_bigint_src", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictFieldFkCustomBigintSource {
...
#[model(table = "strict_field_fk_custom_uuid_src", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
pub struct StrictFieldFkCustomUuidSource {
...
#[djogi::djogi_test(sync_models = [StrictDefaultOff])]
    ...
    let id_checks: Vec<String> = ctx
        .raw_rows(
            "SELECT conname::text FROM pg_constraint \
             WHERE conrelid = 'strict_default_off'::regclass \
             AND contype = 'c' \
             AND conname LIKE 'strict_default_off_id%' \
             ORDER BY conname",
            &[],
        )
        .await
...
#[djogi::djogi_test(sync_models = [StrictStrictHeer])]
    ...
    let defs: Vec<String> = ctx
        .raw_rows(
            "SELECT pg_get_constraintdef(oid)::text FROM pg_constraint \
             WHERE conrelid = 'strict_strict_heer'::regclass \
             AND contype = 'c' \
             AND conname = 'strict_strict_heer_id_check'",
            &[],
        )
        .await
        .expect("catalog query must succeed")
...
#[djogi::djogi_test(sync_models = [StrictStrictRanj])]
async fn strict_ranj_id_has_uuidv8_check(mut ctx: djogi::DjogiContext) {
    ...
    let defs: Vec<String> = ctx
        .raw_rows(
            "SELECT pg_get_constraintdef(oid)::text FROM pg_constraint \
             WHERE conrelid = 'strict_strict_ranj'::regclass \
             AND contype = 'c' \
             AND conname = 'strict_strict_ranj_id_check'",
            &[],
        )
        .await
        .expect("catalog query must succeed")
...
#[djogi::djogi_test(sync_models = [StrictFieldOptin])]
async fn field_optin_emits_check_only_on_marked_column(mut ctx: djogi::DjogiContext) {
    ...
    let constraint_names: Vec<String> = ctx
        .raw_rows(
            "SELECT conname::text FROM pg_constraint \
             WHERE conrelid = 'strict_field_optin'::regclass \
             AND contype = 'c' \
             ORDER BY conname",
            &[],
        )
        ...
        .collect();
    assert!(
        constraint_names.contains(&"strict_field_optin_external_owner_check".to_string()),
        "field-level opt-in must emit CHECK on `external_owner`; got: {constraint_names:?}"
    );
    assert!(
        !constraint_names.contains(&"strict_field_optin_id_check".to_string()),
...
#[djogi::djogi_test(sync_models = [StrictFkTarget, StrictFkSource])]
async fn fk_propagation_emits_check_on_fk_column(mut ctx: djogi::DjogiContext) {
    ...
    let defs: Vec<String> = ctx
        .raw_rows(
            "SELECT pg_get_constraintdef(oid)::text FROM pg_constraint \
             WHERE conrelid = 'strict_fk_source'::regclass \
             AND contype = 'c' \
             AND conname = 'strict_fk_source_owner_id_check'",
            &[],
        )
        .await
        .expect("catalog query must succeed")
...
async fn strict_heer_id_valid_round_trip(mut ctx: djogi::DjogiContext) {
    ...
    let row = StrictStrictHeer::create(
        &mut ctx,
        StrictStrictHeer {
    ...
    let fetched = StrictStrictHeer::get(&mut ctx, row.id)
        .await
...
async fn strict_ranj_id_valid_round_trip(mut ctx: djogi::DjogiContext) {
    let row = StrictStrictRanj::create(
        &mut ctx,
        StrictStrictRanj {
    ...
    let fetched = StrictStrictRanj::get(&mut ctx, row.id)
        .await
...
async fn strict_heer_check_rejects_negative_id(mut ctx: djogi::DjogiContext) {
    ...
    let err = ctx
        .raw_execute(
            "INSERT INTO strict_strict_heer (id, created_at, updated_at, label) \
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("strict_strict_heer_id_check"),
...
async fn strict_ranj_check_rejects_uuid_v4(mut ctx: djogi::DjogiContext) {
    ...
    let err = ctx
        .raw_execute(
            "INSERT INTO strict_strict_ranj (id, created_at, updated_at, label) \
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("strict_strict_ranj_id_check"),
        "rejection must cite the strict_id_check constraint: {msg}"
    );
}
...
async fn strict_fk_rejects_negative_owner_id(mut ctx: djogi::DjogiContext) {
    ...
    let err = ctx
        .raw_execute(
            "INSERT INTO strict_fk_source (id, created_at, updated_at, owner_id, label) \
    ...
    let msg = format!("{err:?}");
    assert!(
        msg.contains("strict_fk_source_owner_id_check"),
...
#[djogi::djogi_test(
    sync_models = [StrictCustomBigintTarget, StrictCustomBigintFkSource]
)]
async fn fk_to_custom_bigint_target_skips_strict_check(mut ctx: djogi::DjogiContext) {
    ...
    //
    // The `sync_models` macro stubs the `strict_custom_bigint_id_next`
    // function via the typed surface; we only assert the constraint
    // catalog state here, not the function existence.
    let id_checks: Vec<String> = ctx
        .raw_rows(
            "SELECT conname::text FROM pg_constraint \
             WHERE conrelid = 'strict_custom_bigint_fk_source'::regclass \
             AND contype = 'c' \
             AND conname LIKE 'strict_custom_bigint_fk_source_owner_id%' \
             ORDER BY conname",
            &[],
        )
        .await
    ...
    let col_type: String = ctx
        .raw_rows(
            "SELECT format_type(atttypid, atttypmod)::text \
             FROM pg_attribute \
             WHERE attrelid = 'strict_custom_bigint_fk_source'::regclass \
...
#[djogi::djogi_test(
    sync_models = [StrictCustomUuidTarget, StrictCustomUuidFkSource]
)]
async fn fk_to_custom_uuid_target_skips_strict_check(mut ctx: djogi::DjogiContext) {
    ...
    let id_checks: Vec<String> = ctx
        .raw_rows(
            "SELECT conname::text FROM pg_constraint \
             WHERE conrelid = 'strict_custom_uuid_fk_source'::regclass \
             AND contype = 'c' \
             AND conname LIKE 'strict_custom_uuid_fk_source_owner_id%' \
             ORDER BY conname",
            &[],
        )
        .await
    ...
    let col_type: String = ctx
        .raw_rows(
            "SELECT format_type(atttypid, atttypmod)::text \
             FROM pg_attribute \
             WHERE attrelid = 'strict_custom_uuid_fk_source'::regclass \
             AND attname = 'owner_id'",
            &[],
        )
        .await
...
#[djogi::djogi_test(
    sync_models = [StrictCustomBigintTarget, StrictFieldFkCustomBigintSource]
)]
async fn field_level_fk_to_custom_bigint_skips_strict_check(mut ctx: djogi::DjogiContext) {
    ...
    let id_checks: Vec<String> = ctx
        .raw_rows(
            "SELECT conname::text FROM pg_constraint \
             WHERE conrelid = 'strict_field_fk_custom_bigint_src'::regclass \
             AND contype = 'c' \
             AND conname LIKE 'strict_field_fk_custom_bigint_src_owner_id%' \
             ORDER BY conname",
            &[],
        )
        .await
...
#[djogi::djogi_test(
    sync_models = [StrictCustomUuidTarget, StrictFieldFkCustomUuidSource]
)]
async fn field_level_fk_to_custom_uuid_skips_strict_check(mut ctx: djogi::DjogiContext) {
    ...
    let id_checks: Vec<String> = ctx
        .raw_rows(
            "SELECT conname::text FROM pg_constraint \
             WHERE conrelid = 'strict_field_fk_custom_uuid_src'::regclass \
             AND contype = 'c' \
             AND conname LIKE 'strict_field_fk_custom_uuid_src_owner_id%' \
             ORDER BY conname",
            &[],
        )
        .await
