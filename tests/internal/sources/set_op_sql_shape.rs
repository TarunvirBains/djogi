...
#[model(table = "set_op_authors", pk = HeerId)]
#[derive(Debug, Clone)]
pub struct Author {
    pub name: String,
}
...
// chain is exercised.
#[model(table = "set_op_books", pk = HeerId, no_default)]
#[derive(Debug, Clone)]
pub struct Book {
    pub title: String,
...
#[model(table = "set_op_places", pk = HeerId, no_default)]
#[derive(Debug, Clone)]
pub struct Place {
    pub name: String,
...
fn set_op_none_arm_short_circuits_to_where_false() {
    ...
    assert!(
        sql.contains("FROM set_op_widgets WHERE FALSE"),
