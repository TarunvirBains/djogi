...
async fn advisory_lock_same_bucket_blocks_different_does_not(mut ctx: djogi::DjogiContext) {
  //: Postgres advisory locks are reentrant within a single
...
// ── an internal review umbrella: --squash refuses when dev_mode is off ─────────────

/// an internal review umbrella BLOCK: `attune --squash` must refuse when
...
// ── an internal review umbrella: --squash refuses when DJOGI_ENV=production ────────

/// an internal review umbrella BLOCK: `attune --squash` must refuse when the
...
// ── an internal review umbrella: gate ordering — localhost evaluated first ─────────
...
async fn attune_squash_refuses_on_missing_from_version(mut ctx: djogi::DjogiContext) {
  ...
  let err = attune(&mut ctx, req).await.expect_err("must refuse");
  match err {
    // After, an absent version is reported via the more
...
async fn attune_squash_collapses_local_files(mut ctx: djogi::DjogiContext) {
  ...
  // start out describing the PRE-squash file content; requires
  ...
  //: the retained `from` row's `checksum_up` and `description`
...
// ── regression: padded `=` in libpq form refuses squash ──────────────
...
// ── regression: DiffOnly does NOT bootstrap the ledger ────────────────
...
// ── regression: disk scan filters by current_database() ──────────────
...
// ── regression: squash refuses when --from spans multiple buckets ────
...
// ──: explicit --app scopes the squash to a single bucket ─────────────
...
// ──: --publish path against a fixture remote ─────────────────────────
...
async fn attune_squash_with_publish_pushes_to_remote_origin(mut ctx: djogi::DjogiContext) {
  //: prove the `--publish` path actually shells out to
  ...
  // row is needed for the checksum refresh; the two later rows
  ...
  //: a single `attune --squash --publish` invocation
...
// ── an internal review umbrella: target / --apply / --record live coverage ────────
...
/// an internal review umbrella: when `target` is supplied, attune resolves it
...
/// an internal review umbrella: a target that does not exist locally AND has
...
/// an internal review umbrella: `--apply false` must keep Record mode read-only.
...
async fn u1_attune_record_without_apply_is_dry_run(mut ctx: djogi::DjogiContext) {
  ...
  // an internal review umbrella the dry-run path also does not bootstrap the
  // ledger table on a fresh per-test DB — so the absence-of-row
  // assertion takes the form "either the table is missing entirely
  // (no bootstrap on dry-run, the invariant) or the table is
  // present but the version is absent". Both shapes prove no row
  // was inserted on this run; the dedicated tests pin the
...
/// an internal review umbrella: `--apply true` mutates as before. Same
...
/// an internal review umbrella: `--record --apply` with a resolved target
...
/// an internal review umbrella: `--record` without `--apply` is a dry-run for
...
// ── an internal review umbrella: dry-run must NOT bootstrap ledger table ───────────

/// an internal review umbrella BLOCK: `attune --record-ledger` (Record mode)
...
/// an internal review umbrella: `attune --record-ledger --apply` on a fresh DB
...
/// an internal review umbrella: `attune --squash` without `--apply` MUST NOT
...
// ── an internal review umbrella: --squash implies recording ───────────────────────

/// an internal review umbrella round: `--squash` clearly implies
...
/// an internal review umbrella round: the dry-run side of the auto-imply
...
async fn u7_attune_squash_dry_run_surfaces_record_skipped_without_explicit_flag(
  ...
  assert!(
    ...
    "U-7 +: Squash dry-run must surface DryRunRecordSkipped with the \
     resolved target SHA: {:?}",
    report.diagnostics
  );
