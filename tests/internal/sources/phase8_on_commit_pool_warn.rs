// Phase 8α T2.7 — `on_commit` on a pool-backed `DjogiContext` is an
...
// The Phase 8 §D3 canonical sequence (`before → DB → outbox → after →
// on_commit drain`) only fires inside `atomic()` (or an explicit
// `commit()` on a transaction-backed context). T2.7 mirrors the
// Phase 5 `_insecurely` audit-warn pattern: pool-backed callers emit
...
/// This pins the existing Phase 4 behavior so the T2.7 audit-warn does
