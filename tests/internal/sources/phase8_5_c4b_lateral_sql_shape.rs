...
#[model(table = "phase8_5_c4b_lateral_other_parents", pk = HeerId)]
#[derive(Debug, Clone)]
...
fn test_inner_lateral_shape() {
    ...
    assert!(
        sql.contains("FROM phase8_5_c4b_lateral_parents WHERE active = $1"),
...
fn test_left_lateral_none_inner_forces_empty_subquery() {
    ...
    assert!(
        sql.contains("FROM phase8_5_c4b_lateral_children WHERE FALSE"),
...
fn test_nested_subquery_in_lateral_inner_keeps_outer_ref_scope() {
    ...
    assert!(
        sql.contains(
            "EXISTS (SELECT 1 FROM phase8_5_c4b_lateral_children WHERE parent_id = l.id)"
        ),
...
fn test_count_lateral_shape() {
    ...
    assert!(
        sql.contains(
            "FROM phase8_5_c4b_lateral_parents ORDER BY name ASC LIMIT $1 OFFSET $2) AS l"
        ),
...
fn test_outer_distinct_on_is_preserved_inside_derived_source() {
    ...
    assert!(
        sql.contains("FROM phase8_5_c4b_lateral_parents ORDER BY name ASC) AS l"),
...
fn test_outer_binds_precede_inner_binds_after_renumbering() {
    ...
    assert!(
        sql.contains("FROM phase8_5_c4b_lateral_parents WHERE active = $1 LIMIT $2"),
