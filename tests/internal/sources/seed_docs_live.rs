...
// ── an internal review round: per-database URL routing ──────────────────────────
//
...
// ── an internal review umbrella: db reset replay must honour historical apply order ─

/// an internal review umbrella (BLOCK): `db reset` must replay migrations in
...
/// this proves the full reset replay chain honours end-to-end.
///
...
// ── an internal review umbrella PARTIAL: db seed --database <other_db> live route ─────

/// an internal review umbrella PARTIAL: prove that `db seed --database <other_db>`
/// actually runs SQL against `<other_db>`, NOT the application
/// database. Pre-- the `--database` flag selected the seed
