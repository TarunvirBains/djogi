// Phase 8.5 Cluster 2 issue #118 — `db reset` replay writes
// `djogi_ddl_audit` rows when `ResetRequest::audit_pool` is `Some(..)`.
//
// # What this test proves
//
// Pre-fix:
// - `RunnerCtx::audit_pool` plumbing existed in Cluster 8ε (T9.4 / T9.5)
...
// virgin DB, composes a Phase 0 migration, calls `reset_app_database`
...
/// Compose Phase 0 against `virgin_db` so the workspace carries one
...
async fn db_reset_with_audit_pool_writes_djogi_ddl_audit_rows() {
    ...
    // 2. Build a workspace and compose Phase 0 for that virgin DB.
    ...
    assert!(
        ...
        "Phase 0 must be replayed (control: same shape as the existing \
         phase8_zero_db_reset_replays_phase_zero test)"
    );
    ...
    //     DB with the empty (global) app label. The Phase 0 migration
    ...
    assert!(
        row_count >= 1,
        "expected at least one djogi_ddl_audit row after Phase 0 replay; got {row_count}"
    );
