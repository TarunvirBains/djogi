// Phase 8.5 Cluster 4 (djogi#213) — typed Postgres network family
...
#[model(table = "phase8_5_c4_213_network_rows", pk = HeerId, no_default)]
#[derive(Debug, Clone, PartialEq)]
pub struct Phase85C4213NetworkRow {
    pub host: IpAddr,
...
#[djogi::djogi_test(sync_models = [Phase85C4213NetworkRow])]
async fn network_ipv4_round_trip(mut ctx: djogi::DjogiContext) {
    ...
    let row = Phase85C4213NetworkRow::create(
        &mut ctx,
        Phase85C4213NetworkRow {
    ...
    let fetched = Phase85C4213NetworkRow::get(&mut ctx, row.id)
        .await
...
async fn network_filter_by_inet_returns_matching_row(mut ctx: djogi::DjogiContext) {
    ...
    let results = Phase85C4213NetworkRow::objects()
...
async fn network_bulk_update_sets_host(mut ctx: djogi::DjogiContext) {
    ...
    let n = Phase85C4213NetworkRow::objects()
