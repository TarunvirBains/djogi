...
/// translation from the v3 `IndexTarget` shape to the simple
...
///
/// This convention is consumed by the migration emitter when it
...
/// `Condition::Expr` so the spatial predicate routes through the /// expression substrate rather than introducing a parallel `Condition::Spatial`
...
fn places_migration_shape_splits_gist_index_out_of_transaction() {
    ...
    // The contract test documents this choice so knows the canonical case.
    assert!(
