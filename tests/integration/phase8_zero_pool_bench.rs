...
//!    workload against a 1-slot pool.
...
///
/// Assertion: 64-worker run completes faster than 1-worker run on the
/// same total op count. We don't assert exact speed-up ratios — flaky
/// on shared CI runners. The point is that the pool is not silently
/// serializing.
#[tokio::test]
async fn bench_max_size_scaling() {
    ...
    // Concurrency assertion — same total op count in both conditions,
    // 64-worker should beat 1-worker on wall-clock if the pool is
    // genuinely overlapping IO across slots.
    let serial = wall_per_width
        .iter()
        .find(|(w, _, _)| *w == 1)
        .copied()
        .expect("serial run recorded");
    let parallel = wall_per_width
        .iter()
        .find(|(w, _, _)| *w == 64)
        .copied()
        .expect("64-worker run recorded");

    println!(
        "serial 1-worker total = {:.2}ms; \
         64-worker total = {:.2}ms (same {} total ops in both)",
        serial.1.as_secs_f64() * 1000.0,
        parallel.1.as_secs_f64() * 1000.0,
        serial.2,
    );

    assert!(
        parallel.1 < serial.1,
        "64-worker wall-clock ({:?}) should beat 1-worker wall-clock ({:?}) \
         on identical total op count — pool is not delivering concurrency",
        parallel.1,
        serial.1,
    );
