...
#[model(table = "interval_rows", pk = HeerId, no_default)]
...
#[djogi::djogi_test(sync_models = [IntervalRow])]
async fn interval_mixed_components_round_trip(mut ctx: djogi::DjogiContext) {
    ...
    let fetched = IntervalRow::get(&mut ctx, row.id)
        .await
...
async fn interval_independent_components_round_trip(mut ctx: djogi::DjogiContext) {
    ...
    let months_row = IntervalRow::create(
        &mut ctx,
        IntervalRow {
            id: <::djogi::types::HeerId as ::djogi::PrimaryKey>::sentinel(),
            created_at: ::djogi::types::DateTime::UNIX_EPOCH,
    ...
    let days_row = IntervalRow::create(
        &mut ctx,
        IntervalRow {
            id: <::djogi::types::HeerId as ::djogi::PrimaryKey>::sentinel(),
            created_at: ::djogi::types::DateTime::UNIX_EPOCH,
...
async fn interval_present_not_in_empty_excludes_null_rows(mut ctx: djogi::DjogiContext) {
    ...
    let results = IntervalRow::objects()
...
async fn interval_boundary_components_round_trip(mut ctx: djogi::DjogiContext) {
    let min_row = IntervalRow::create(
        &mut ctx,
        IntervalRow {
            id: <::djogi::types::HeerId as ::djogi::PrimaryKey>::sentinel(),
            created_at: ::djogi::types::DateTime::UNIX_EPOCH,
    ...
    let max_row = IntervalRow::create(
        &mut ctx,
        IntervalRow {
            id: <::djogi::types::HeerId as ::djogi::PrimaryKey>::sentinel(),
            created_at: ::djogi::types::DateTime::UNIX_EPOCH,
...
async fn interval_bulk_update_increments_and_decrements_duration(mut ctx: djogi::DjogiContext) {
    ...
    let incremented = IntervalRow::objects()
    ...
    let current = IntervalRow::get(&mut ctx, row.id)
        .await
    ...
    let decremented = IntervalRow::objects()
        .filter(|f| f.id().eq(row.id))
    ...
    let updated = IntervalRow::get(&mut ctx, row.id)
        .await
...
async fn interval_sql_eq_linearizes_months_and_days(mut ctx: djogi::DjogiContext) {
    ...
    let results_by_month = IntervalRow::objects()
    ...
    let results_by_days = IntervalRow::objects()
    ...
    let results_thirty_one = IntervalRow::objects()
