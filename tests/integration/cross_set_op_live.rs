...
#[derive(Debug, Clone)]
pub struct Login { pub actor: String, pub occurred: i32 }

#[model(table = "x462_live_edits", pk = HeerId)]
#[derive(Debug, Clone)]
pub struct Edit { pub actor: String, pub occurred: i32 }

#[model(table = "x462_live_activity", pk = HeerId)]
#[derive(Debug, Clone)]
pub struct Activity { pub actor: String, pub occurred: i32 }

async fn seed(ctx: &mut djogi::DjogiContext) {
    for (actor, occurred) in [("ann", 10), ("bob", 20), ("cat", 30)] {
        Login::create(ctx, Login { actor: actor.to_string(), occurred, ..Default::default() })
            .await.expect("seed Login");
    }
    for (actor, occurred) in [("ann", 40), ("dan", 50)] {
        Edit::create(ctx, Edit { actor: actor.to_string(), occurred, ..Default::default() })
            .await.expect("seed Edit");
    }
}
...
async fn cross_union_all_merges_two_models(mut ctx: djogi::DjogiContext) {
    ...
    let rows: Vec<Activity> =
        djogi::query::union_all_as::<Activity, _, _>(Login::objects(), Edit::objects())
            .fetch_all(&mut ctx).await.unwrap();
...
async fn cross_union_all_outer_order_and_limit_apply(mut ctx: djogi::DjogiContext) {
    ...
    let rows: Vec<Activity> =
        djogi::query::union_all_as::<Activity, _, _>(Login::objects(), Edit::objects())
            .order_by("occurred", OuterOrder::Desc).limit(2)
            .fetch_all(&mut ctx).await.unwrap();
...
async fn cross_first_returns_outer_ordered_row(mut ctx: djogi::DjogiContext) {
    ...
    let row: Option<Activity> =
        djogi::query::union_all_as::<Activity, _, _>(Login::objects(), Edit::objects())
            .order_by("occurred", OuterOrder::Asc).first(&mut ctx).await.unwrap();
...
async fn cross_arm_with_lock_rejected_at_terminal(mut ctx: djogi::DjogiContext) {
    ...
    let err = djogi::query::union_as::<Activity, _, _>(plain, locked)
        .fetch_all(&mut ctx).await.unwrap_err();
...
#[derive(Debug, Clone)]
pub struct TLogin { pub org_id: String, pub actor: String }

#[model(table = "x462_live_tedits", pk = HeerId, tenant_key = "org_id")]
#[derive(Debug, Clone)]
pub struct TEdit { pub org_id: String, pub actor: String }

#[model(table = "x462_live_tactivity", pk = HeerId, tenant_key = "org_id")]
#[derive(Debug, Clone)]
pub struct TActivity { pub org_id: String, pub actor: String }
...
async fn cross_tenant_wiring_sets_guc_and_does_not_poison(mut ctx: djogi::DjogiContext) {
    ...
    djogi::transaction::atomic(&mut ctx, |ctx| Box::pin(async move {
        for actor in ["ann", "bob"] {
            TLogin::create(ctx, TLogin { org_id: "org_a".to_string(), actor: actor.to_string(), ..Default::default() })
                .await.expect("seed TLogin");
        }
        TEdit::create(ctx, TEdit { org_id: "org_a".to_string(), actor: "cat".to_string(), ..Default::default() })
            .await.expect("seed TEdit");

        let rows: Vec<TActivity> =
            djogi::query::union_all_as::<TActivity, _, _>(TLogin::objects(), TEdit::objects())
                .fetch_all(ctx).await.unwrap();
