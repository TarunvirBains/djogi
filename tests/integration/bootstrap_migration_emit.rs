//! `migrations compose` auto-emits
//! a bootstrap migration for any database that doesn't already
//! have one.
//!
//! # What this proves
//!
...
fn compose_auto_emits_bootstrap_with_postgis_dependency_on_first_run() {
    ...
    assert!(
        ...
        "PostGIS dependency must propagate into bootstrap migration"
    );
...
fn compose_auto_emit_is_idempotent_across_runs() {
    ...
    assert!(
        second.is_empty(),
        "second compose must NOT re-emit bootstrap migration (got: {second:?})"
    );
...
fn compose_auto_emit_returns_emissions_in_report() {
    ...
    // pre-emits for the database; compose surfaces this as
    // a successful report (NOT NothingToCompose) per the
    ...
    assert_eq!(
        ...
        1,
        "bootstrap migration must be auto-emitted"
    );
    assert_eq!(report.emitted_phase_zero[0].database, "main");

    // Run compose again — already on disk, no delta changes
...
fn compose_auto_emit_aggregates_extensions_across_apps_in_same_database() {
    ...
    assert_eq!(
        emitted.len(),
        1,
        "one bootstrap migration per database (main)"
    );
    let main_emit = &emitted[0];
    assert_eq!(main_emit.database, "main");
    assert!(
        main_emit.extensions.contains("postgis"),
        "billing's PostGIS dep must aggregate into main's bootstrap migration"
    );
    assert!(
        main_emit.extensions.contains("pg_trgm"),
        "shipping's pg_trgm dep must aggregate into main's bootstrap migration"
    );
