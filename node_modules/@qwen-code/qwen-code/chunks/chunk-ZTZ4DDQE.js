...
init_esbuild_shims();
var debugLogger = createDebugLogger("IMAGE_TOKENIZER");
var ImageTokenizer = class _ImageTokenizer {
  static {
    __name(this, "ImageTokenizer");
  }
  ...
  extractWebpDimensions(buffer) {
    ...
    if (format === "VP8 ") {
      ...
      const height = (buffer.readUInt32LE(26) & 16777215) + 1;
      return { width, height };
    }
  ...
  calculateTokensWithScaling(width, height) {
    ...
    const minPixels = _ImageTokenizer.MIN_TOKENS_PER_IMAGE * _ImageTokenizer.PIXELS_PER_TOKEN;
    const maxPixels = _ImageTokenizer.MAX_TOKENS_PER_IMAGE * _ImageTokenizer.PIXELS_PER_TOKEN;
    ...
    const imageTokens = Math.floor(
      hBar * wBar / _ImageTokenizer.PIXELS_PER_TOKEN
    );
    return imageTokens + _ImageTokenizer.VISION_SPECIAL_TOKENS;
  }
  /**
  ...
  async calculateTokensBatch(base64DataArray) {
    ...
    for (const { data, mimeType } of base64DataArray) {
      try {
        ...
        results.push(
          _ImageTokenizer.MIN_TOKENS_PER_IMAGE + _ImageTokenizer.VISION_SPECIAL_TOKENS
        );
      }
    }
    return results;
...
// packages/core/src/utils/request-tokenizer/requestTokenizer.ts
var debugLogger2 = createDebugLogger("TOKENIZER");
var RequestTokenizer = class {
  static {
    __name(this, "RequestTokenizer");
  }
  textTokenizer;
  imageTokenizer;
  constructor() {
  ...
  async calculateTokens(request) {
    ...
    try {
      ...
      if (textContents.length === 0 && imageContents.length === 0 && audioContents.length === 0 && otherContents.length === 0) {
        return {
          totalTokens: 0,
          breakdown: {
            textTokens: 0,
      ...
      debugLogger2.error("Error calculating tokens:", error);
  ...
  async calculateTextTokens(textContents) {
    ...
    try {
      ...
      debugLogger2.warn("Error calculating text tokens:", error);
  ...
  async calculateImageTokens(imageContents) {
    ...
    try {
      ...
      debugLogger2.warn("Error calculating image tokens:", error);
      return imageContents.length * 6;
    }
  }
  /**
  ...
  async calculateAudioTokens(audioContents) {
    ...
    for (const audioContent of audioContents) {
      try {
        ...
        debugLogger2.warn("Error calculating audio tokens:", error);
        totalTokens += 10;
      }
    }
    return totalTokens;
  ...
  async calculateOtherTokens(otherContents) {
    ...
    try {
      ...
      debugLogger2.warn("Error calculating other content tokens:", error);
  ...
  calculateFallbackTokens(request) {
    try {
      ...
      debugLogger2.warn("Error in fallback token calculation:", error);
      return 100;
    }
  }
  /**
  ...
  processPart(part, textContents, imageContents, audioContents, otherContents) {
    ...
    try {
      ...
      debugLogger2.warn("Failed to serialize unknown part type:", error);
    }
  }
};
