...
var CronCreateInvocation = class extends BaseToolInvocation {
  ...
  async execute() {
    ...
    try {
      ...
      let llmContent;
      if (recurring) {
        llmContent = `Scheduled recurring job ${job.id} (${job.cronExpr}). Session-only (not written to disk, dies when Qwen Code exits). Auto-expires after 3 days. Use CronDelete to cancel sooner.`;
      } else {
        llmContent = `Scheduled one-shot task ${job.id} (${job.cronExpr}). Session-only (not written to disk, dies when Qwen Code exits). It will fire once then auto-delete.`;
      }
