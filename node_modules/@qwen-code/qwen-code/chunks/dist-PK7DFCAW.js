...
`);
  ...
  onPromptStart(chatId) {
    ...
    const sendTyping = /* @__PURE__ */ __name(() => this.bot.api.sendChatAction(chatId, "typing").catch(() => {
    }), "sendTyping");
    sendTyping();
    this.typingIntervals.set(chatId, setInterval(sendTyping, 4e3));
  }
  onPromptEnd(chatId) {
  ...
  buildEnvelope(msg, text, entities) {
    ...
    const isMentioned = entities?.some((e) => e.type === "mention" && this.botUsername && text.slice(e.offset, e.offset + e.length).toLowerCase() === `@${this.botUsername.toLowerCase()}`) ?? false;
