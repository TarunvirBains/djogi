...
var FatalToolExecutionError = class extends FatalError {
  static {
    __name(this, "FatalToolExecutionError");
  }
  constructor(message) {
    super(message, 54);
  }
...
function _resetValidatePathCacheForTest() {
...
var GOOGLE_ACCOUNTS_FILENAME2 = "google_accounts.json";
...
var Storage = class _Storage {
  ...
  static runWithRuntimeBaseDir(dir, cwd, fn) {
    ...
    return _Storage.runtimeBaseDirContext.run(resolved, fn);
  }
  /**
  ...
  static getRuntimeBaseDir() {
    ...
    if (contextualDir !== void 0) {
      return contextualDir ?? _Storage.getGlobalQwenDir();
    }
  ...
  static getGoogleAccountsPath() {
    return path2.join(_Storage.getGlobalQwenDir(), GOOGLE_ACCOUNTS_FILENAME2);
  }
  ...
  getProjectDir() {
    ...
    const projectsDir = path2.join(
      _Storage.getRuntimeBaseDir(),
      PROJECT_DIR_NAME
    );
...
function resetDebugLoggingState() {
  hasWriteFailure = false;
  ensureDebugDirPromise = null;
  ensuredDebugDirPath = null;
}
__name(resetDebugLoggingState, "resetDebugLoggingState");
...
function updateLatestDebugLogAlias(sessionId) {
  ...
  void ensureDebugDirExists().then(() => updateSymlink(aliasPath, targetPath, { fallbackCopy: false })).catch(() => {
  });
}
...
function runWithDebugLogSession(session, fn) {
