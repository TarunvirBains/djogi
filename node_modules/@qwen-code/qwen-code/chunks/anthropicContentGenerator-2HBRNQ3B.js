...
async function fileFromPath2(path, ...args) {
  const { fileFromPath: _fileFromPath } = await import("./fileFromPath-IBEHA3CO.js");
...
function buildAnthropicUsageMetadata(parts) {
  ...
  const looksLikeOpenAi = cacheCreationTokens === 0 && cacheReadTokens > 0 && inputTokens >= cacheReadTokens;
...
var AnthropicContentConverter = class {
  ...
  convertGeminiRequestToAnthropic(request, options = {}) {
    ...
    const system = enableCacheControl ? this.buildSystemWithCacheControl(systemText, useGlobalCacheScope) : systemText;
  ...
  async convertGeminiToolsToAnthropic(geminiTools, options = {}) {
    ...
    if (enableCacheControl && tools.length > 0) {
      ...
      tools[lastToolIndex] = {
        ...tools[lastToolIndex],
        cache_control: useGlobalCacheScope ? { type: "ephemeral", scope: "global" } : { type: "ephemeral" }
      };
    }
    return tools;
  }
...
// packages/core/src/core/anthropicContentGenerator/anthropicContentGenerator.ts
var debugLogger = createDebugLogger("ANTHROPIC");
...
var AnthropicContentGenerator = class {
   ...
   */
  useGlobalCacheScope() {
    return this.contentGeneratorConfig.enableCacheControl !== false && isAnthropicNativeBaseUrl(this.contentGeneratorConfig);
  }
  /**
  ...
  resolveEffectiveEffort(request) {
    ...
    if (effort === "max" && !isDeepSeekAnthropicHostname(this.contentGeneratorConfig)) {
      if (!this.effortClampWarned) {
        debugLogger.warn(
          "reasoning.effort='max' is a DeepSeek extension; clamping to 'high' for non-DeepSeek anthropic provider to avoid HTTP 400."
        );
  ...
  buildThinkingConfig(request, effectiveEffort) {
    ...
    const budgetTokens = effectiveEffort === "low" ? 16e3 : effectiveEffort === "max" ? 128e3 : effectiveEffort === "high" ? 64e3 : 32e3;
    return {
      type: "enabled",
    ...
    for await (const event of stream) {
      switch (event.type) {
        ...
        case "content_block_start": {
          ...
          blocks.set(index, {
            type,
            id: "id" in event.content_block ? event.content_block.id : void 0,
            name: "name" in event.content_block ? event.content_block.name : void 0,
