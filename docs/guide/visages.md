...
payloads, shared contracts, and frontend code. Phase 4.5 introduced
visages as output-shape types (with `From` / `TryFrom` conversions);
Phase 7-Zero-2 promoted them to first-class **query entities** with
compile-time scope enforcement and SELECT narrowing. Field exposure is
...
**Relation form (Phase 7-Zero-2 grammar with `->` traversals).** A
...
// Nested `expose(...)` declarations on the embedded struct, one
// scope at a time. Empty `expose()` body means "use this struct's
// own per-field expose annotations as-is".
#[field(expose(public -> Owner { expose() }))]
pub owner: ForeignKey<Owner>,
```

Three exposure levels per FK: ID-only (no `->`), narrow peer visage
(`-> Visage`), or full-struct (`-> ModelStruct`) optionally with
nested `expose(...)` declarations. The peer name must be a type
...
relations now project as `Option<PeerVisage>` and participate in
traversal. Phase 4.5's "deferred to a follow-up phase" rejection no
longer applies.
...
**Reverse-FK and M2M boundaries are symmetrical.** From a peer
visage, the reverse accessor returns a `VisageQuerySet` of the
visage-scoped child type:

```rust
let public_address: AddressPublic = /* ... */;
let owners: Vec<PublicRegisteredOwner> = public_address
    .public_registered_owners()      // returns VisageQuerySet<PublicRegisteredOwner>
...
The Phase 7-Zero-2 grammar covers the boundary-enforcement and
...
current grammar is a strict subset of the larger grammar those
phases will enable.
