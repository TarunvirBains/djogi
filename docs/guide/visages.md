...
-> Owner { expose() })`) are **rejected by the parser** in this
phase — the brace-suffix grammar is reserved for a follow-up phase
and currently emits an actionable compile error. The peer name must be a type
