...
For relation field types, eager loading, and explicit-through many-to-many declarations, see the [relations guide](./relations.md). For typed JSONB, spatial types, full-text search, and other rich field types, see the corresponding feature guides.
...
Controls the primary key type and generation strategy. The value is a **bare identifier**, not a string literal (`pk = HeerId`, not `pk = "heerid"`). Omitting `pk` resolves to `HeerIdRecencyBiased` (recency-biased default).
...
Only valid on `ForeignKey<T>` fields (). Consumed by migration projection/DDL for the emitted foreign-key action.

---

## Field Types

 maps these Rust types to SQL column types:
...
The following Postgres column types ship as first-class Rust shapes (
 — djogi#170 umbrella). The Rust-side codecs are hand-rolled against
...
|---|---|---|---|
| `INTERVAL` | `djogi::Interval { months, days, microseconds }` | — (always on) | djogi#212 |
| `int4range` / `int8range` / `numrange` / `tsrange` / `tstzrange` / `daterange` | `djogi::Range<i32>` / `djogi::Range<i64>` / `djogi::Range<rust_decimal::Decimal>` / `djogi::Range<time::PrimitiveDateTime>` / `djogi::Range<djogi::DateTime>` / `djogi::Range<djogi::Date>` | — (always on) | djogi#215 |
| `INET` | `std::net::IpAddr` | `network` | djogi#213 |
| `CIDR` | `djogi::CidrAddr { addr, prefix }` | `network` | djogi#213 |
| `MACADDR` | `djogi::MacAddr([u8; 6])` | `network` | djogi#213 |
...
stored as `INET` rather than `CIDR`) are presently out of scope. Adopters use `djogi::CidrAddr` and a `CIDR` column, or declare the column as `Custom("INET")` + an adopter newtype that preserves the prefix.
...
|---|---|---|---|
| `MONEY` | Presently uses `NUMERIC` | Use `rust_decimal::Decimal` with a `NUMERIC` column; `Custom("MONEY")` for legacy columns | djogi#214 |
| Postgres multirange types | Presently covers single-range columns only | `Custom("int4multirange")` / sibling SQL type plus adopter newtype | — |
| `DOMAIN` declarations — Piece B (`CREATE DOMAIN` emission via `#[model(domains = [...])]`) | Piece A shipped; Piece B deferred | Declare the domain in raw SQL (adopter-owned bootstrap migration) and reference it via `#[field(domain = "<name>")]` (Piece A) | djogi#216 |
...
djogi does NOT emit `CREATE DOMAIN` DDL; the domain must already exist on the target database.
...
use `FieldSqlType::Custom("public.positive_amount")`.
...
round-trip through a domain-wrapped column does NOT work for stock Rust types — postgres-types' `ToSql` impl for
...
- `domain + generated` — generated columns derive their storage
 type from the expression; combining with a domain type is not supported.
