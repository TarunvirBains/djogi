...
**`HeerId`:** 64-bit `BIGINT`, ascending / time-ordered. Generated by `generate_id()` at the database level.
...
Generated SQL column: `id BIGINT PRIMARY KEY DEFAULT generate_id()`.
...
Generated SQL column: `id UUID PRIMARY KEY DEFAULT generate_ranjid()`. Note: the SQL function is `generate_ranjid` (no underscore), not `generate_ranj_id`.
