...
- Djogi's descriptor-driven migration flow includes the tenant RLS policy for
  the model. Use `djogi migrations compose` for generated migration
   ...
   `djogi::migrate` library APIs when applying from code). The `rollback` and `baseline` CLI dispatchers are deferred; `apply --fake` ships as a flag on the apply command. Hand-written RLS
