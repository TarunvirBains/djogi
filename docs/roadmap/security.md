...
2. Include RLS policy SQL in descriptor-driven migration composition
...
Running `djogi migrations compose` shows the generated policy SQL before it is applied:

```sql
-- Generated by: djogi rls
...
For RLS to work, every database transaction that accesses tenant-scoped tables must set the tenant context before any query fires. When a context carries an `AuthContext` with `tenant_id`, tenant-keyed CRUD/QuerySet operations auto-issue `set_tenant`; code that does not attach auth context must set it explicitly inside the transaction.
...
- Shortcuts to avoid setting up `DjogiContext::set_tenant(...)` or auth-context auto-tenant scoping
...
When the migration library updates `schema_snapshot.json`, it also writes an HMAC-SHA256 signature of the file contents. On the next build, the build script verifies the signature before trusting the snapshot. If the signature does not match, the build fails with a clear error.
...
async fn get_invoice(Path(id): Path<HeerIdRecencyBiased>, State(pool): State<DjogiPool>) -> impl IntoResponse {
    let mut ctx = DjogiContext::from_pool(pool.clone());
    let invoice = Invoice::objects_insecurely()
        .filter(|f| f.id().eq(id))
        .fetch_one(&mut ctx).await?;  // any user can access any org's invoice
    Json(invoice)
}
```
