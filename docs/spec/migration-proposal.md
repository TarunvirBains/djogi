...
CREATE TABLE IF NOT EXISTS djogi_schema_migrations (
    -- Surrogate PK (allows multiple rows per version when failure rows are retained)
    ...
    status                TEXT          NOT NULL DEFAULT 'pending'
                              CHECK (status IN (
                                  'pending', 'applied', 'baseline', 'faked',
                                  'out_of_order', 'rolled_back', 'failed'
                              )),
...
|---|---|---|---|
| `id` | Missing (version was PK) | `BIGINT IDENTITY PRIMARY KEY` | OI-05 — allows failure rows per version |
| `status` | Not present | 7-variant CHECK enum | R-06 — pre-write row eliminates crash window |
...
- `out_of_order` — applied out of version sequence; `out_of_order_flag` also set.
