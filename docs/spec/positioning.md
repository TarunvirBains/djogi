...
| Spatial | PostGIS behind `spatial` feature; `GeoPoint` / `LineString` / `Polygon` / `MultiPoint` / `MultiPolygon` field lowering with EWKB codecs (`MultiLineString` codec exists but field lowering is not yet wired); `convex_hull` aggregate plus `area_of` / `area_of_intersection` scalar expressions | Not surfaced | sqlx types only | Not surfaced |
...
- **SQL migration files** — chosen for operator transparency, replica safety, and DBA-readability over Rust-code migrations. Data-only or out-of-band operations (e.g. a backfill `UPDATE` alongside descriptor-generated DDL) live in author-edited `.sql` files, which re-compose preserves by default — it refuses to overwrite any up or down file whose bytes differ from what the deterministic emitter would freshly produce; `--force-overwrite` is the explicit opt-in that discards hand-edits and restores canonical SQL. The default is preservation, not a Rust-attribute escape hatch. Cot's choice on this axis is more strictly Rust-first (migrations *are* Rust code there).
