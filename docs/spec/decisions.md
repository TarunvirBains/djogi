...
| Spatial region-JOIN function choice (Phase 6.5) | `group_by_region` / `count_by_region` emit `LEFT JOIN … ON ST_Covers(r.<r-geo-col>, t.<t-geo-col>)` — not `ST_Contains`. `ST_Covers(geography, geography)` exists in PostGIS 3.x (`ST_Contains` does not) and gives identical semantics for point-in-polygon queries. Using `ST_Covers` keeps both sides in the geography type family so GiST-indexed bbox prefiltering stays active under the JOIN. |
...
| Aggregate alias discipline (Phase 6.5) | Aggregates are aliased `__djogi_agg_N` (0-indexed). User-supplied SELECT aliases that collide with the `__djogi_agg_*` namespace are rejected at SQL-build time with `DjogiError::AnnotationAliasCollision`. The synthetic alias prevents user renames from breaking positional decode. |
