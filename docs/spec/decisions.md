...
| Regex in framework code | **Prohibited.** Djogi uses byte-level character-class checks (`u8::is_ascii_alphabetic`, `u8::is_ascii_alphanumeric`, explicit byte equality), sorted const slices with `binary_search`, and other primitive techniques. No `regex`, `regex-lite`, `fancy-regex`, `regex-automata`, or other regex-engine dependency may enter the workspace — not in `djogi`, `djogi-macros`, `djogi-cli`, `djogi-shell`, or any future crate. Regex is heavy, hides intent behind a DSL most readers re-parse every time, and invites per-query allocation. Stdlib byte logic is explicit, zero-dependency, and compiles to tight code. The prohibition applies to implementation code only; doc comments may use regex notation (e.g. `[A-Za-z_]`) as a human-readable shorthand for a rule that is actually implemented with byte checks. |
