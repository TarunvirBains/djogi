...
| Spatial codec | Manual EWKB encode/decode in-repo (`djogi/src/geo/ewkb.rs`) — 21-byte little-endian layout (endian marker, type word with SRID flag, SRID, X longitude, Y latitude). No new dependency. A future dep swap to `geozero` or equivalent is a single-file change because all raw-byte logic is isolated in `ewkb.rs`. |
