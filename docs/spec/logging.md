...
Each target owns its own ledger, snapshot, and advisory-lock scope. Djogi may later coordinate ordered multi-target workflows, but it does not claim distributed atomic migration across the app, CRUD-log, and event-log databases. The `rollback`, `repair`, and `baseline` migration dispatchers are deferred CLI surfaces; use the public `djogi::migrate` library APIs for those operations today. `apply` ships as `djogi migrations apply` and `verify` ships as `djogi migrations verify`.
