...
| `E_DJG_VDF_009` | `sql` contains an aggregate or window-function call from the recognised aggregate-name set (`COUNT`, `SUM`, `AVG`, `MIN`, `MAX`, `ARRAY_AGG`, `STRING_AGG`, `JSONB_AGG`, `JSON_AGG`, `JSONB_OBJECT_AGG`, `JSON_OBJECT_AGG`, `RANGE_AGG`, `MULTIRANGE_AGG`, `XMLAGG`, `BOOL_AND`, `BOOL_OR`, `EVERY`, `BIT_AND`, `BIT_OR`) at top level, or the `OVER` keyword. Detection is best-effort: aggregates outside this set or custom user-defined aggregates pass macro parsing and surface as Postgres errors at query time. | offending function call position |
...
- `compile_fail`: one fixture per `E_DJG_VDF_*` macro-time error,
  with span-anchored expected diagnostic.
