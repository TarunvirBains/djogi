...
This document maps common app patterns to the earliest Djogi phase at which they are "safe to adopt" — meaning the needed primitives ship and are stable enough for production app code.
...
| Transactions + `atomic` / `on_commit` / savepoints | 4 | `DjogiContext` + `atomic()` |
| Row locks + bulk upsert / bulk update | 4 | `select_for_update` + `bulk_upsert` |
...
| Projection structs for HTTP boundaries | 4.5 | `#[field(expose)]` + generated DTOs |
| Dirty tracking + optimistic locking | 5 | `Tracked<T>` + `#[field(version)]` |
| Typed Postgres enums | 5 | `#[derive(DjogiEnum)]` |
| Array fields with native operators | 5 | `contains` / `contained_by` / `overlap` / `len` |
| `Jsonb<T>` with unknown-field preservation | 5 | Both flat `.path::<V>("...")` and typed `#[derive(JsonbSchema)]` |
