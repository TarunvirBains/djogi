...
| Production migrations | 7 | Descriptor differ + `djogi migrations apply`/`compose`/`status`/`attune`/`verify`/`repair`/`baseline`, and target-scoped ledger/snapshot; `rollback` CLI dispatcher is deferred |
