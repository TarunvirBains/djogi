...
| Production migrations | 7 | Descriptor differ + `djogi migrations apply`/`compose`/`status`/`attune`, and target-scoped ledger/snapshot; verify/repair/baseline CLI dispatchers are deferred |
