...
| `DjogiEnum` () | Checkbox group of variants  |
...
A third behavior (paginated browse with no typeahead, virtualized infinite scroll, etc.) is out of v1 — adopters can extend through the future `AdminFieldWidget` surface, anchored to 0.5 (see [Phase Map](./phase-map.md)).
...
**Out of v1.** Filters that need access to the parent form's *in-progress state* (e.g., filter children by the parent's tenant value before save) are not v1 — `DjogiContext` already carries authenticated user, role, and tenant, but not in-flight form values. 0.5 candidate; the signature would extend with an additional parameter once the pattern is documented.
...
| `GeoPoint` () | Map widget with click-to-set + manual lat/lng inputs |
...
- **Inline create/delete accounting and the approval threshold.** Each inline-row creation or removal is accounted as an individual `Create` or `Delete` action on the through model. A magnitude threshold reroutes mass-removal inline saves through the dual-control approval flow as a distinct `InlineSave` pending action — a sibling to `BulkDelete` that shares the same approval queue, lifecycle, and dual-control discipline (see `operations.md` for the shared table and lifecycle). This closes a bypass: without the threshold, an operator could perform the same volume of deletes via inline editing as via a changelist `BulkDelete` and skip approval.
 ...
 - The threshold applies only to **removals**. Inline creates and inline updates do not contribute to the threshold; they remain normal-save-flow regardless of count. (High-volume inline-create/update gating is a 0.5 candidate if a deployment needs it.)
