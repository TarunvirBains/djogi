...
- Visage-grant RBAC with single-parent inheritance — `_admin_role_visage_perms` per `(role, app, model, visage)`; visages remain pure compile-time projections, sourced from the canonical visages emitted by `expose(...)` annotations (no custom user-defined visage structs in v1 — see 0.5 deferral below)
...
0.5 layers compliance polish atop 0 without breaking changes:
...
- **Component re-use across `dioxus-desktop`.** The renderer choice keeps the desktop path open. Whether 0 publishes a desktop-shell crate, leaves it to adopters, or formally defers to a follow-up phase is open.
