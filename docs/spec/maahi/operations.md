...
4. Assign a role whose **effective visage-grant set** — the union of `_admin_role_visage_perms` rows resolved recursively through `parent_role_id` inheritance, with `can_view` / `can_edit` carried per row — is not a subset of the holder's own effective visage-grant set. A grant on `(app, model, visage_name, can_view = TRUE)` in the assigned role must be matched by the same row (or a strictly stronger one) in the holder's chain; same for `can_edit`. A holder who can see `VehicleAdmin` cannot manufacture a user who sees `VehicleAdminLite` if `VehicleAdminLite` is outside the holder's own grants — this prevents the holder from materializing a user who sees fields the holder cannot. The subset check is computed against effective grants (after `view_full_struct` / `write_full_struct` expansion in clause 2), so a holder who effectively sees the full struct (via that system permission) trivially covers any specific visage grant.
