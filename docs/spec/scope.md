...
4. It composes cleanly with `tokio-postgres`, Postgres, and any Rust web framework (`axum` is the best-covered example today, behind the `axum` feature flag) rather than re-owning their jobs.
