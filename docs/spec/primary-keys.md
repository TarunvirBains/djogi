...
HeerId is a database-native 64-bit time-ordered identifier. IDs are generated by a Postgres function (`heerid_next()`) installed by HeeRanjId's migration, not in application code. The ascending HeerId column default is:
...
```sql
id UUID PRIMARY KEY DEFAULT ranjid_next()
```
