...
The same `version` stays occupied in the ledger until repair resolves the row in place, so another apply of the same version still collides on the unique `(version, app_label)` constraint for that app's stream. Use `djogi migrations status` to inspect the row, then resolve it with `repair_partial_apply` or, when the row is still resumable, `repair_resume_partial_apply`.
...
- further apply work for the same `version` still collides on the unique `(version, app_label)` constraint for that app's stream until the row is repaired in place
