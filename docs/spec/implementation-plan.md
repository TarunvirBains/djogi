...
- [ ] **Retrofit existing examples to use djqry once available.** The Phase 8-Zero Cluster D mating-pairs demo (`examples/elephant-tracker/src/demos/mating_pairs.rs`) ships a `ctx.raw_rows`-shaped Wright F + spatial-overlap query because the typed `AnnotatedQuerySet<T, A>` surface is bolted to a single `Model T` and the demo's `(female, male)` candidate-pair shape doesn't fit (tracked separately as #84). Once `djqry` lands, that call site is the canonical retrofit candidate: extract the SQL into `djqry/elephant_mating_pairs.sql`, surface it as `ElephantDjqry::mating_pairs(&mut ctx).await?`, and update the demo to call the typed method instead. The demo's pedagogical role then shifts from "raw-SQL escape hatch" to "djqry-override surface" — preserving the declarative call-site shape adopters expect across the rest of the example. The same retrofit pattern applies to any future demo or adopter code path that today uses `raw_query` for shape reasons rather than performance reasons
