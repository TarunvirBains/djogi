...
3. **Raw SQL escape hatch ships in ** — the framework must never trap the developer
4. **Tests against real Postgres** — no mocking the database; use the `#[djogi_test]` harness (which spins up a per-test database and runs the runtime helper Phase 0 install with the default node seed/GUCs). The earlier-phase mention of `sqlx::test` is historical — `sqlx` was retired in.
...
**Deliverable:** Macro architecture decision recorded, with narrowed to an implementable slice.
...
- [ ] Minimal slice: support the default HeerId primary key path first
...
** amendment (2026-04-16):** canonicalizes `sqlx::Executor<Database = Postgres>` as the connection abstraction for all CRUD operations. Later phases may introduce a thin Djogi-owned adapter (e.g. `DjogiContext`, planned in ) if transaction-context policies or pool safety guards require more structure — but such an adapter must extend, not replace, the `Executor` contract. No `DjogiConnection` type exists in.

** amendment (2026-04-19):** Per v3's Q1 resolution, the `sqlx::Executor` contract was **replaced** (not extended) by `&mut DjogiContext`. `DjogiContext` carries either a pool or an active transaction and pattern-matches on the inner variant at each sqlx boundary. Every /2/3 CRUD / QuerySet / relation signature retrofitted accordingly; the retrofit shipped as a single concentrated commit on `phase4-retrofit`.
...
## : Transactions & Expressions

**Goal:** Application-level transactions and field expressions for complex queries.

 is the main query/write power inflection point. It is where Djogi stops being "typed CRUD plus a builder" and starts owning the concurrency and expression substrate needed for serious production workloads.
...
This phase owns transport-safe contract generation. It does not replace 's query-time typed result shaping for aggregates, annotations, or other performance-sensitive query outputs.
...
**Goal:** First-class support for Postgres features that other ORMs treat as optional.
 ...
 takes the substrate and makes Postgres-native performance features first-class: typed JSONB and arrays, native aggregates, RLS hooks, index metadata, and typed field families whose schema intent must survive migration/runtime behavior.
...
## : Visage Query Surface + PK Refinements *(shipped)*

**Goal:** Freeze the PK substrate and make visages first-class query
entities before the migration engine and later consumer phases
...
### 7a: Visage Query Surface + Boundary Enforcement *(shipped)*

 shipped visages as output-shape types only: `{Model}Public` / `SelfView` / `Admin` / `Export` carry `impl TryFrom<&Model>` for conversion but have no query-side surface. makes visages first-class query entities with compile-time scope enforcement, filling 's explicit M2M visage deferral in the process.
...
- [ ] Interaction with §9b: b's Tier 1 over-fetching detector gains a concrete suggestable fix — "you hydrated `RegisteredOwner` but only read `.display_name` + `.email` — swap for `PublicRegisteredOwner`" *(belongs to b; left unchecked here as forward-reference)*
...
## : Auth Substrate *(shipped)*

**Goal:** Framework-owned auth primitives that pair with 's tenant-key RLS and 's transaction substrate.
...
**Ordering.** 6.5 ships **before**. The coupling is the descriptor contract (typed `GeographySubtype` discriminant on `FieldSqlType::Geography`, `IndexSpec` usage patterns) — contracts freeze upstream of their consumers, so 6.5 locks the final shape and 's migration differ is designed once against it. 's v3 plan must absorb 6.5's final descriptor shape; no " follow-up" task lives inside 6.5.

**SRID 4326 stays locked — matching.** Arbitrary-SRID generalization is roadmap work (see `docs/roadmap/future-work.md` §4.6), explicitly kept out of 6.5 to avoid the ergonomic tax of const-generic SRIDs on the 95% of users who only need WGS84.
...
- [ ] Shape predicates: `.contains` / `.intersects` / `.touches` / `.within(geometry)` — distinct from 's radius-based `within_km`; dispatch on `FieldRef<M, G: GeographyValue>` via a new sealed `GeographyValue` trait
...
- [ ] `#[djogi_test(extensions = ["postgis"])]` attribute argument auto-provisions extensions at per-test DB creation time — removes the per-test `ctx.raw_ddl("CREATE EXTENSION...")` pattern from 
...
Filenames are `V<YYYYMMDDHHMMSS>__<slug>.sdjql` plus `.down.sdjql`. Every path keys per `(database, app)` bucket. SQL seeds (not Rhai) — the Rhai shell is a separate phase.
...
's migration CLI is explicitly target-scoped. App, CRUD-log, and event-log databases each have their own migration ledger, snapshot, and advisory-lock boundary. Cross-database foreign keys are rejected rather than modeled as first-class relations.
...
- [ ] Lock-timeout on DDL statements so blocked migrations back off rather than queue behind long transactions (`SET lock_timeout = '5s'` around each DDL) — folded into 's online-safety classification work
- [ ] Two-phase column rename: emit `ADD COLUMN new_name` + backfill from `old_name` + runtime reads both + drop `old_name` in a follow-up migration. Driven by `#[field(renamed_from = "old_name")]` + an opt-in `#[field(rename_strategy = "two_phase")]` — (`ExpandContract` classification)
...
## : Live-Migration Substrate + Protected Data Metadata & Field Codecs *(substrate shipped; operator runner deferred)*
...
**Goal:** Add descriptor-level protected-field semantics and storage transforms; add the live-migration substrate that classifies schema operations by online-safety and prepares safe rollout plans. Full operator-driven live execution (`djogi live run/resume/finalize`, status, abandon, daemon resume) is documented but deferred in v0.1.0.
 ...
 expanded beyond the original protected-data scope to absorb the live-migration safety classifier (per the v3 plan). The shipped scope is the classifier/planning/protected-field substrate:
 ...
 *(admin-defaults integration deferred to 0 / Maahi)*
...
- [x] T12 integration tests + per-phase compile-fixture dispatcher
 (originally a trybuild driver; migrated to lihaaf in )

**Deferred:**

- T13 (catalog drift detection / runtime-vs-spec divergence audit) — pushed to or a dedicated bug-fix sprint
...
## : Hooks, Composition, Proxy, Computed Properties, Q-Algebra & Punnu
...
- **During development through 8η**, djogi consumes sassi via a path dependency (`sassi = { path = "../sassi" }`) so the two repos can co-evolve.
- **At publish housekeeping**, sassi v0.1.0 publishes to crates.io first; djogi then flips its Cargo.toml to `sassi = "0.1"`, re-runs its full test sweep against the published crate to confirm no path-vs-published divergence, then publishes djogi v0.1.0.

v0.1.0 ships when **both** are true: (1) djogi complete through 8η plus publish housekeeping merged with green CI, including the post-flip re-test; (2) sassi v0.1.0 live on crates.io. The publish order within is sassi → djogi; that ordering is what the gate enforces.
...
### : Pre-publish housekeeping (v0.1.0 gate)
...
** close → v0.1.0 publish (gated on sassi v0.1.0 also being live on crates.io).**

> ** cluster status note (2026-05-08).** α–8ζ have shipped on
> `main`. The pre-publish housekeeping originally sketched in 8g has been
> refactored into a dedicated **** alpha-readiness cluster, while the
> remaining predicate/cache correctness work is now scoped to a separate
> **eta (8η)** cluster — a correctness cluster, not release
> housekeeping. eta reconciles the Sassi/Djogi predicate model (see
> *Predicate Portability* below), targets the predicate hard-gate issues
> (djogi #121, #126, #127, #107,
> #108, #109; sassi #15), and reduces the issue surface before 8.5. 
...
## : Shell, Analyzer & djqry

**Goal:** Interactive Rhai REPL, static query analyzer, and the djqry SQL override registry.

These surfaces are intentionally downstream of the core ORM/runtime. They are useful operational tools, but they must remain feature-gated adapters over the model/query layer rather than redefining the core identity of the crate. (The admin console is its own phase — see 0 / Maahi.)

** amendment (2026-05-10).** Four architectural decisions land alongside the original task list:
...
2. **djogi-as-dylib is a load-bearing dependency**, shared with lihaaf. The shell binary is small (~5 MB) and dlopens `libdjogi.so` (~30 MB) at startup. See `docs/spec/decisions.md` *Djogi-as-dylib for shell + lihaaf ()* row and `docs/spec/shell.md` §13.11 for the rationale (build iteration, plugin ecosystem, memory hygiene, distribution size — explicitly NOT runtime query speed).
...
- [x] **Coordination with lihaaf:** lihaaf consumes the same dylib for its own reasons; spike runs once and feeds both and lihaaf's v0.1 spec ([`docs/spec/lihaaf-v0.1.md`](./lihaaf-v0.1.md), TBD)
- [x] **Contingency outcome (selected 2026-05-10):** `GO_NATIVE`. Other outcomes named for revalidation completeness (per `docs/spec/shell.md` §13.13):
 ...
 - `NO_GO` — ships statically-linked; dylib-dependent items deferred until toolchain blocker resolves (parse-vs-eval split, djqry authoring loop, ergonomics work all still ship; only the dylib-dependent items defer)

**Phase ordering (sequencing constraints).** milestones depend on upstream work landing in a specific order:

- **ε wrap (set_role + snapshot signing + djogi verify) must complete before §9a implementation begins.** Rationale: the shell uses `DjogiContext`'s post-8ε surface for transaction-scoped role propagation in REPL transactions. §9a code that reaches into a pre-8ε `DjogiContext` would need rewrite when 8ε lands
- **`lihaaf v0.1` spec must land with self-review and independent review before any precompiled-Rhai-plugin (§9a-Plugins) work begins.** Rationale: the cross-harness boundary (lihaaf is Rust-only; owns Rhai test fixtures) needs both specs in agreement before either harness commits to ownership
...
- **§9b (Static Query Analyzer) and §9c (djqry SQL override registry) have no dependency on the dylib coupling.** They can land in parallel with §9a once ε wraps

### 9a: Shell (Rhai REPL)

- [ ] `djogi shell` — launches REPL with all models loaded
- [ ] **Shell binary dlopens `libdjogi.so` at startup** (gated on 9 spike outcome — `NO_GO` falls back to static linking for v0)
...
- [ ] Build-time generation: a new stage in the existing `build.rs` pipeline (alongside `target/djogi_models.json` emission) parses every `.sql` file, validates frontmatter against descriptor metadata, and emits a generated `{Model}Djqry` zero-sized type per owner with one associated async function per override. Call site reads `VehicleDjqry::expired_registrations(&mut ctx).await?` — parallel to 's `{Model}Filter` and 's `{Model}Related` generated types, which is the established convention for per-model namespaced helpers. The `Djqry` suffix is distinctive, grep-able, and zero collision risk. For `@on: _global` overrides the parallel type is `GlobalDjqry`: `GlobalDjqry::fleet_stats(&mut ctx).await?`
...
- [ ] Runtime dispatch: each generated method routes through `ctx.raw_query::<T>(...)` ( substrate) and decodes via `FromPgRow`. An override-firing tracing event names the override so 1b / 11e observability surfaces highlight hand-tuned queries distinctly from macro-generated ones
...
**Deliverable:** Working shell, `djogi analyze query` lint pass, and `djqry` SQL override registry surfaced as typed model methods with a shell-native authoring loop. (Admin console — Maahi — is 0.)
...
## 0.5: Maahi Compliance & Delegation

**Goal:** Layer compliance and delegation polish on 0 v1 without breaking changes. Brings Maahi to enterprise-compliance grade and Django-parity feature breadth.
...
- [ ] 0's admin layer (Maahi) surfaces slow-query log, pool stats, long-running transactions, recent `crud_logs` entries for a given record — provided the observability hooks from 11b/11c/11d are wired
...
- [ ] Per-model autovacuum tuning: `#[model(autovacuum = VacuumPolicy::HighChurn)]` emits per-table `ALTER TABLE... SET (autovacuum_vacuum_scale_factor =...,...)` as DDL routed through 's migration generation pipeline. 1.5 provides the policy vocabulary + CLI/ops surface; owns the DDL emission and phased execution
...
## 2: Distributed Topology & Residency
...
## 3: Runtime Query Verification (D7xx)
...
| 9: Shell, Analyzer & djqry | Medium | Interactive tools (admin console split out to 0 / Maahi) |
...
**The critical path to standing alongside popular Rust ORM alternatives is Phases 0–4.** improves contract hygiene and shared contract reuse without changing that write-path boundary. Phases 5–13 add the Postgres-native depth, governance, scale-oriented capabilities, and projection-correctness audit needed for broader high-scale confidence.
