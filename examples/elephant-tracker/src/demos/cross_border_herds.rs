...
// JUSTIFICATION: temporary debt. Grouped M2M aggregation must move to the typed surface rather than remain a raw-SQL showcase.
pub async fn run(ctx: &mut DjogiContext, format: Format, out: Option<&Path>) -> Result<()> {
    ...
    // explicit-through M2M still needs a typed replacement.
