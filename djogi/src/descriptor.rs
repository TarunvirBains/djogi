...
/// Metadata for a single model field — user-defined only.
///
/// Framework-injected fields (`id`, `created_at`, `updated_at`) do NOT
/// appear in `ModelDescriptor::fields`: consumers that need them (the
/// migration differ, admin UI) know to synthesize the column metadata
/// themselves from `pk_type`. Keeping framework fields out of `fields`
/// means length / iteration / diffing matches the user's mental model of
/// "my fields".
#[derive(Debug, Clone)]
...
/// generation (Phase 5), partitioning (Phase 7), the outbox system (Phase 5).
/// Extending this struct is a coordinated action across all call sites —
/// every `inventory::submit!` generated by the macro must fill every field.
#[derive(Debug)]
