...
fn emit_alter_column(table: &str, column: &str, change: &ColumnChange) -> OperationSql {
    ...
    let (up, down, label_suffix, lossy) = match change {
        ...
        // proper `ALTER COLUMN ADD/DROP/SET GENERATED` syntax. Postgres
        // 10+ identity columns are sequence-backed; ADD triggers
        // sequence allocation and starts the sequence after MAX(c) for
        // existing rows, so no Djogi-side backfill is needed.
        ColumnChange::SetIdentity { from, to } => {
            ...
            match (from, to) {
                (None, Some(kind)) => {
                    ...
                    let up = format!("ALTER TABLE {qt} ALTER COLUMN {qc} ADD {clause};");
                ...
                (Some(prev), None) => {
                    ...
                    let down = format!("ALTER TABLE {qt} ALTER COLUMN {qc} ADD {clause};");
