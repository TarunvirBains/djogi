...
fn project_column(
    ...
    // means:
    //   1. The differ correctly detects identity additions and emits
    //      `ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY`,
    //      which is the right Postgres syntax. Inlining into sql_type
    //      would route through `ColumnChange::ChangeType` and emit
    //      `ALTER COLUMN id TYPE INTEGER GENERATED BY DEFAULT AS IDENTITY`
    //      which is invalid SQL.
    //   2. Snapshot round-trip stays clean — the sql_type comparison is
    //      "INTEGER" both before and after the fix; only the new
