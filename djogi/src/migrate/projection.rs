...
fn project_column(
    ...
    //
    // Serial-PK id-column carve-out (#86): the framework's typed
    // surface declares `pk = Serial` adopters get an auto-incrementing
    // INTEGER primary key (per `docs/spec/primary-keys.md`), but the
    // descriptor emits `INTEGER` as the id field's sql_type with no
    // sequencing clause, and `pk_default_sql(Serial)` returns `None`
    // because IDENTITY is part of the column type, not a DEFAULT
    // expression. Without this carve-out, the emitted DDL is
    // `id INTEGER NOT NULL` and INSERTs without an explicit id fail.
    // FK columns *referencing* a Serial PK must NOT get the IDENTITY
    // clause — only the PK column itself owns the sequence — so the
    // FK-substitution path above is intentionally untouched; this
    // override fires only for the model's own `id` column when the
    // model's PK is `Serial`.
    let sql_type = if f.relation_kind.is_some()
        && let Some(target) = f.target_type_name
        && let Some(pk_sql) = type_to_pk_sql.get(target)
    {
        pk_sql.clone()
    } else if f.name == "id" && matches!(parent.pk_type, PkType::Serial) {
...
mod tests {
    ...
    /// #86 — Serial PK must emit auto-increment IDENTITY clause.
    ///
    /// Pre-fix: the projection emitted `id INTEGER NOT NULL` with no
    /// DEFAULT and no IDENTITY, which made INSERT-without-explicit-id
    /// fail Postgres's NOT NULL check. The fix inlines
    /// `GENERATED BY DEFAULT AS IDENTITY` into the id column's
    /// `sql_type` for Serial PKs only — FK columns referencing a
    /// Serial PK keep plain `INTEGER` (sequence ownership lives on
    /// the PK column, not its references).
    #[test]
    fn serial_pk_emits_identity_clause_on_id_column() {
        static FIELDS: &[FieldDescriptor] = &[FieldDescriptor {
        ...
        assert_eq!(
            id_col.sql_type, "INTEGER GENERATED BY DEFAULT AS IDENTITY",
            "Serial PK id column must inline IDENTITY clause; got {}",
            id_col.sql_type
        );
        // default_sql stays None — IDENTITY is part of the column type,
        // not a DEFAULT expression. The DDL emitter at sql.rs:1228 only
        // appends ` DEFAULT {def}` when default_sql is Some.
        assert_eq!(
    ...
    /// PK column itself, not on its FK references; emitting
    /// `country_id INTEGER GENERATED BY DEFAULT AS IDENTITY` on a
    /// child table would create a duplicate sequence and corrupt the
    /// reference semantics.
    #[test]
    fn serial_pk_fk_references_stay_plain_integer() {
        ...
        assert_eq!(
            ...
            "FK to Serial PK must NOT inherit IDENTITY clause; got {}",
