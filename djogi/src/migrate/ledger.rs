...
//! );
//! ```
//! Version uniqueness is scoped per app stream: the composite constraint
//! `UNIQUE (version, app_label)` means two buckets with different app
//! labels can independently apply at the same version. The empty
//! `app_label` is the synthetic global bucket — not a sentinel for
//! "unscoped".
...
/// repeated calls a no-op. Routes through `DjogiContext::raw_ddl`
/// which uses Postgres's simple query protocol (DDL statements that
/// the prepare-cache cannot handle).
///
/// Also handles the per-app version-stream upgrade: pre-existing alpha
/// ledgers carry a column-level UNIQUE constraint on `version` alone.
/// Bootstrap replaces it with a composite `UNIQUE (version, app_label)`
/// constraint so multiple buckets can independently apply at the same
/// version. Both steps are conditional on catalog state so repeated
...
mod tests {
    ...
    /// Pre-existing alpha ledgers have the column-level UNIQUE (version)
    /// constraint. After bootstrap, the composite (version, app_label)
    /// constraint must be in place and accept rows with the same version
    /// but different app labels.
    #[djogi_test]
    async fn bootstrap_upgrades_old_constraint_to_composite(mut ctx: DjogiContext) {
        // Create the old-shape table with a column-level UNIQUE on version.
        let old_ddl = r#"
            CREATE TABLE djogi_schema_migrations (
                id                    BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                ...
                execution_mode        TEXT        NOT NULL DEFAULT 'transactional'
                              CHECK (execution_mode IN ('transactional', 'non_transactional')),
                status                TEXT        NOT NULL DEFAULT 'pending'
                              CHECK (status IN ('pending', 'applied', 'baseline',
                                                'faked', 'rolled_back', 'failed')),
                applied_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
                applied_by            TEXT        NOT NULL DEFAULT current_user,
                execution_time_ms     BIGINT      NOT NULL DEFAULT 0,
        ...
        let cs = compute_checksum(["CREATE TABLE t ()"]);
        insert_pending(
            &mut ctx,
            &LedgerRow {
                version: version.to_string(),
                description: "test a".to_string(),
                checksum_up: cs.clone(),
                checksum_down: None,
        ...
        insert_pending(
            ...
            &LedgerRow {
                ...
                run_id: 2,
                snapshot_version: "1".to_string(),
                app_label: "system".to_string(),
                leaf_identity: None,
            },
        )
        .await
        .expect("second row inserts (same version, different app)");

        // Verify both rows are present.
        let rows = select_all(&mut ctx).await.expect("read ledger");
